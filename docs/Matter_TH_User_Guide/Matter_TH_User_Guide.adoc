...
:th-version: v2.15-beta2+spring2026
...
IMPORTANT: This workflow should be treated as a temporary workaround until native OTA image generation support is added to the Test Harness.

=== High-Level Workflow Steps

ATLs should follow these steps when preparing for OTA-related test cases:

. Identify that OTA-related test cases require an OTA requestor image (`ota-requestor-app.ota`), which is not auto-generated by the Test Harness.
...
[source,shell]
----
cp ota-requestor-app.ota ~/apps/images/
----
...
* This is a **temporary workaround** until the Test Harness includes native OTA image generation support.
