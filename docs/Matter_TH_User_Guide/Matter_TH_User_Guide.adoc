...
The chip-camera-app that is shared internally is compiled for arm64 and should be executed in a machine that is in the same network as the Test Harness. To run the camera application, use the linux commands below:
