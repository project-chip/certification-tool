...
| Matter 1.4.2       | v2.13+summer2025   | N/A                                                                                         | https://groups.csa-iot.org/wg/members-all/document/folder/4651[Causeway Link] | a12250a      | To install, use the tag "v2.13-beta4+summer2025" in the instructions of <<fresh_install>>
| Matter 1.5     | v2.14-beta1.1+winter2025   | N/A                                                                                         |  | 7b24545      | To install, use the tag "v2.14-beta1.1+winter2025" in the instructions of <<fresh_install>>
|===
...
1. Open the Chrome browser and type and go to `chrome://flags/ in a new tab
...
Auto-update and start Test Harness with the camera feature (`v2.14-beta1.1+winter202` or later) with the following linux commands: 
...
Since the Test Harness IP address can be used to launch the TH user interface on any system on the same network as the Test Harness using the browser. Then, you may use the camera application to run a camera related Test Case.

==== Running chip-camera-app with USB Camera

The chip-camera-app that is shared internally is compiled for arm64 and should be executed in a machine that is in the same network as the Test Harness. To run the camera application, use the linux command below:

1. `rm -rf /tmp/chip_*`
2. `./chip-camera-app` (use `./chip-camera-app --camera-deferred-offer` for the TC_WEBRTC_1_3 test case)
