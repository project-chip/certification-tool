...
[WARNING]
====
Known problem: The latest version of wpa_supplicant (v2.11 or later) may cause compatibility issues with certain Wi-Fi chipsets on Raspberry-Pi. This can result in the wireless connection intermittently dropping and reconnecting.
...
[NOTE]
It is required to run on commissionable device before running each test
...
**Reason**: There is a protecion in SDK that it will stop sending PAF packets if the device is scanning. The period is of 15 seconds. If the environment is noisy, it's possible to hit the problem and fail to continue.

**Solution/WAR**: Need a cleaner environment. We will add a patch to make this period configurable in SDK.

* *Device failed to run after upgrading to v2.10 wpa_supplicant*:

**Symptom**:Device can associate with AP, but fails at the last step if running Wi-Fi commissioning (including code-wifi, ble-wifi, wifipaf-wifi)

**Reason**:The problem will be observed if the `/etc/wpa_supplicant/wpa_supplicant.conf` file has the AP information.
