...
:th-version: v2.16-beta2+summer2026
...
| Matter 1.6     | v2.15+spring2026     | N/A                                                                                         | https://groups.csa-iot.org/wg/members-all/document/folder/2269[Causeway Link] | b91b83f      | To install, use the branch "v2.15+spring2026" in the instructions of <<fresh_install>>
| Matter 1.6.1   | v2.15.1+spring2026   | N/A                                                                                         | TBD | df8bd03      | To install, use the branch "{v2.15.1+spring2026}" in the instructions of <<fresh_install>>
| Matter 1.7     | {th-version}       | N/A                                                                                         | TBD | 99a81bd      | To install, use the branch "{th-version}" in the instructions of <<fresh_install>>
...
| 63          | 20-May-2026  | [Apple]Romulo Quidute               | * Renamed Python Testing Suite categories for clarity: "Python Testing Suite" is now "Python Testing Suite - Auto commissioning" and "Python Testing Suite - No commissioning" is now "Python Testing Suite - No auto commissioning".
...
| 75          | 21-Sep-2026  | [Apple]Romulo Quidute               | * Added PICS Export, Rename, Delete, and Archive/Unarchive Test Run Execution sections to the CLI documentation.
|===
...
. Flash the '{ubuntu-description}' image using one of the following tools on the Mac/PC:
* *Raspberry Pi Imager:* open https://www.raspberrypi.com/software/[Raspberry Pi Imager] and select '{ubuntu-description}' as the operating system (under _Other general-purpose OS_ > _Ubuntu_).
* *Balena Etcher:* download the Raspberry Pi image of '{ubuntu-description}' from https://ubuntu.com/download/raspberry-pi[ubuntu.com/download/raspberry-pi], then open https://www.balena.io/etcher/[Balena Etcher], choose *Flash from file* and select the downloaded image. Skip the OS customization settings below (see the note).
. If using Raspberry Pi Imager, edit the OS customization settings to:
* username: ubuntu
+
...
+
NOTE: The username/password/hostname/SSH customization above is done through Raspberry
Pi Imager's OS customization settings, and enabling SSH with password authentication
there is enough. Balena Etcher only writes the raw image and has no customization step.
An image flashed with Etcher uses the defaults from the `user-data` cloud-init file on
the boot partition: username `ubuntu`, password `ubuntu` (you must change it at first
login), and `ssh_pwauth: false`, so SSH password login is disabled. You can still log
in on the Pi's own display and keyboard. To allow SSH with a password, set
`ssh_pwauth: true` in `user-data` on the boot partition before the first boot.
...
==== Test Harness Startup Hanging with ModuleNotFoundError
...
Currently the OTBR in the TH works with either the Nordic RCP dongle or SiLabs RCP dongle. Refer to <<instructions-to-flash-the-firmware-nrf52840-rcpdongle, Section 7.1>> to flash the NRF52840 firmware or <<instructions-to-flash-silabs-rcp, Section 7.3>> to flash the SiLabs firmware and get the RCP’s ready. Once the RCP’s are programmed, the user needs to insert the RCP dongle on to the Raspberry Pi running the TH and reboot the Raspberry Pi.
...
* SiLabs RCP Firmware: see the ot-rcp-binaries download above
...
| *Raspberry Pi*
| Version permissible for CSA Matter Test Events, used as a platform for Test Harness (Pi 4 or higher).

| https://www.st.com/en/evaluation-tools/stm32wba65i-dk1.html[STM32WBA65I-DK1]
| STMicroelectronics's STM32WBA65I Discovery Kit (mother board)

| X-NUCLEO-NFC11B2
| STMicroelectronics's NFC expansion board. +
Note: This board is not yet available on the market. Please contact tania.guidet@st.com to obtain it.

| HID OMNIKEY 5022 CL
| NFC/RFID contactless reader. Connected to the Raspberry Pi via USB. Host uses the PC/SC driver to communicate with it. +
This NFC Reader shall be used when testing NFC commissioning on a *DUT as Commissionee*.

|===

In case of *'nfc-thread'* commissioning, an nRF52840 Dongle, from Nordic, should also be set up.

The STM32WBA65I-DK1 and X-NUCLEO-NFC11A1-S boards form the reference design for *'nfc-thread'* tests for a *DUT as Commissioner*.

==== Software

STM32WBA65I-DK1 firmware will be available in Matter CSG release directory(https://groups.csa-iot.org/wg/matter-csg/document/folder/3023[matter-csg])

This firmware can be flashed with the tool https://www.st.com/en/development-tools/stm32cubeprog.html[STM32 Cube Programmer].
...
Refer to "_Python Script Validation Procedure_" sheet of TestPlanVerification XLS document (e.g.: TC_DD_1_5, TC_DD_3_22, TC_DD_3_23, TC_DD_3_24).
...
.. Input the DUT configuration details like discriminator: "3840", setup_code:"20202021", and *pairing_mode as "thread-meshcop"*.
+
[source,xml]
----
"dut_config": {
...
. Identify that OTA-related test cases require an OTA requestor image (`ota-requestor-app.ota`).

. Build the OTA Provider applications on the DUT/Reference platform.

. Update the Software Version on the DUT (for example, from the default value 1 to a higher version such as 2) to enable OTA upgrade validation.
...
=== Reference Commands (Raspberry Pi Platform)

NOTE: The commands below are provided as a reference implementation for RPi-based setups. Other platforms may require different build commands.
...
. Edit the configuration file to update the software version:
+
[source,shell]
----
...
. Run the OTA image tool to create the OTA image:
+
[source,shell]
----
...
* `-p 0xBEEF`: Product ID
* `-vn 2`: Version Number
* `-vs "2.0"`: Version String
* `-da sha256`: Digest Algorithm
* `chip-ota-requestor-app`: Input binary file
* `ota-requestor-app.ota`: Output OTA image file

==== Deploy OTA Image to Test Harness
...
The Test Harness implementation relies on a PC/SC (https://en.wikipedia.org/wiki/PC/SC[Personal Computer/Smart Card]) driver. In our setup, the NFC device used for commissioning is exposed and managed as a contactless smart card.
...
Note: The CLI instalation script is automatically executed when setting up the Matter Test-Harness. If you have not set up the Test-Harness yet, refer to <<matter-test-harness-setup, Section 4, Matter Test-Harness Setup>>. Otherwise, you can run the above command to install or update the CLI whenever needed.

. Configure connection to Test-Harness:
+
...
[source,bash]
----
# Display CLI and Test-Harness versions
th-cli --version

# Check Test-Harness runner status
th-cli test-runner-status
----
...
# List tests with compact format
th-cli available-tests --compact

# List tests in JSON format
th-cli available-tests --json

# Filter by cluster
th-cli available-tests --cluster ACE
...
# Run multiple tests
th-cli run-tests --tests-list TC-ACE-1.1,TC-ACE-1.2,TC-DRLK-1.1

# Run with custom title
th-cli run-tests --tests-list TC-ACE-1.1 --title "Door Lock Test Run"
----
...
[source,bash]
----
# Use custom project configuration file
th-cli run-tests --tests-list TC-ACE-1.1 --config my_config.json
...
The mapping file is a JSON object whose keys are TC IDs and whose values are `test_parameters` dicts, using the same format accepted by the project configuration.
...
[source,bash]
----
# Run a single test, supplying its parameters from the mapping file
th-cli run-tests --tests-list TC-ACE-1.3 --project-id 10 -m mapping_tc.json

# Run multiple tests; only the entries present in the mapping file are applied
th-cli run-tests --tests-list TC-ACE-1.1,TC-ACE-1.3,TC-MCORE-FS-1.3 -m mapping_tc.json
----
...
|===
| Source | Description
| Project configuration | Values stored in the TH project (persistent baseline)
| `--tc-params-file` | Parameters from the mapping file — override the project config for matched TC IDs
| `--config` file | Execution-only config file passed via `--config` — overrides the mapping file
| `-- inline args` | Arguments passed after `--` on the command line — always take precedence
|===

This means the mapping file values *win over the project config* but are still overridden by an explicit `--config` file or inline `--` arguments, giving you full flexibility to fine-tune individual runs without touching the project.
...
th-cli project create --name "My DUT Tests"

# Create with project configuration file
th-cli project create --name "Door Lock Project" --config project_config.json
----

NOTE: *Tip:* +
The project configuration structure from an existing project may be verified using the following CLI command `th-cli project list --id <PROJECT-ID> --json`. +
That way it's possile to create new projects and update based on existing configurations.
...
th-cli project list --json

# View specific project details
th-cli project list --id 5
----
...
th-cli project update --id 5 --config updated_config.json
...
th-cli test-run-execution

# List all test runs
th-cli test-run-execution --all

# Sort by ascending order
th-cli test-run-execution --sort asc
----

[#cli-repeat-execution]
==== Repeat a Test Run Execution

The `test-run-execution repeat` subcommand creates a new test run execution with the same selected tests and config as an existing one, then starts it and attaches to it the same way `run-tests` does (and the frontend's "Repeat" action does): streaming live test progress and forwarding any user prompts to this terminal.

[source,bash]
----
# Repeat execution 87 and immediately start/stream it
th-cli test-run-execution repeat --id 87

# Override the generated title (the backend always appends an updated timestamp regardless)
th-cli test-run-execution repeat --id 87 --title my-rerun

# Disable colored output for this execution
th-cli test-run-execution repeat --id 87 --no-color

# Disable the real-time web log viewer for this execution
th-cli test-run-execution repeat --id 87 --no-streaming
----

`--no-streaming` disables the browser-based real-time log viewer (enabled by default, normally reachable at a URL printed at the start of the run); logs are still written to the local log file either way.

[#cli-export-execution]
==== Export a Test Run Execution

The `test-run-execution export` subcommand exports a test run execution's config and results to a JSON file, which can later be imported into another project with `test-run-execution import`.

[source,bash]
----
# Export execution 87 to '<execution-title>-execution.json'
th-cli test-run-execution export --id 87

# Export to a specific output file
th-cli test-run-execution export --id 87 --output-file execution-87.json
----

[#cli-import-execution]
==== Import a Test Run Execution

The `test-run-execution import` subcommand imports a test run execution previously written by `test-run-execution export` into a given project.

[source,bash]
----
# Import a previously exported execution into project 5
th-cli test-run-execution import --file execution-87.json --project-id 5
----

[#cli-pics-export]
==== Export PICS Used by a Test Run Execution

The `test-run-execution pics-export` subcommand exports the PICS actually used by a specific test run execution to a ZIP file.

[source,bash]
----
# Export the PICS used by execution 87 to '<execution-title>-pics.zip'
th-cli test-run-execution pics-export --id 87

# Export to a specific output file
th-cli test-run-execution pics-export --id 87 --output-file execution-87-pics.zip
----

[#cli-rename-execution]
==== Rename a Test Run Execution

The `test-run-execution rename` subcommand renames an existing test run execution.

[source,bash]
----
# Rename execution 87
th-cli test-run-execution rename --id 87 --name "My Renamed Execution"
----

[#cli-delete-execution]
==== Delete a Test Run Execution

The `test-run-execution delete` subcommand permanently deletes a test run execution.

[source,bash]
----
# Delete execution 87 (prompts for confirmation)
th-cli test-run-execution delete --id 87

# Delete execution 87 without a confirmation prompt
th-cli test-run-execution delete --id 87 --yes
----

WARNING: Deletion is permanent and cannot be undone.

[#cli-archive-execution]
==== Archive / Unarchive a Test Run Execution

The `test-run-execution archive` and `test-run-execution unarchive` subcommands mark a test run execution as archived or restore it, without deleting it. Archived executions are hidden from the default `test-run-execution` list; pass `--archived` to view them instead.

[source,bash]
----
# Archive execution 87
th-cli test-run-execution archive --id 87

# Unarchive execution 87
th-cli test-run-execution unarchive --id 87

# List only archived test run executions
th-cli test-run-execution --archived

# List archived executions for a specific project
th-cli test-run-execution --archived --project-id 5
----

NOTE: `--archived` is not applicable when fetching a specific execution via `--id`.
...
. The CLI will display a URL to access the video verification interface
. Open the URL in a web browser (e.g., http://<your-ip>:8999)
. View the live video stream from the camera
. Respond to verification prompts through the web interface
. Video recordings are automatically saved to `./videos/` directory

**Camera Test Requirements:**

* GStreamer must be installed (see <<matter-test-harness-cameras, Section 12, Matter Test Harness Cameras>>)
...
NOTE: For detailed camera setup instructions, refer to <<matter-test-harness-cameras, Section 12, Matter Test Harness Cameras>> in this guide.
...
th-cli run-<TAB>  # Shows: run-tests
----
...
export TH_CLI_NO_COLOR=1
th-cli run-tests --tests-list TC-ACE-1.1
----

NOTE: `--no-color` is also available on `test-run-execution repeat` (see <<cli-repeat-execution, Repeat a Test Run Execution>>).
...
# Check if port 8999 is available
