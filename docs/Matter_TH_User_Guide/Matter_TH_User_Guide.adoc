...
The `test-run-execution repeat` subcommand creates a new test run execution with the same selected tests and config as an existing one, then — by default — starts it and attaches to it the same way `run-tests` does (and the frontend's "Repeat" action does): streaming live test progress and forwarding any user prompts to this terminal.
...
# Only create the repeated execution without starting it
th-cli test-run-execution repeat --id 87 --no-start
