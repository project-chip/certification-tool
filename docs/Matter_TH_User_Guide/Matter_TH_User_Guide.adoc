...
* *‘nfc-thread’* to complete the pairing for the NFC Device using nfc payloads and Thread Device using Border Router.
...
.. To validate test cases using *nfc-thread*, the following parameter must be included in the test parameters, irrespective of the test case-specific arguments:
+
[source,xml]
----
- "int-arg": "NFC_Reader_index:<value>"
e.g., "int-arg": "NFC_Reader_index:0"
----
NFC_Reader_index tells the test which NFC reader hardware to use, and must be set correctly based on the ATL setup
