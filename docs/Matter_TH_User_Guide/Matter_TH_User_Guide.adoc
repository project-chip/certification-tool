...
:th-version: v2.13-beta4.1+summer2025
...
| Matter 1.2       | TH-Fall2023        | https://drive.google.com/file/d/1WTjhc7xbYt18RvpABU3_r47uqOLd7NN1/view?usp=drive_l  ink[link] | https://groups.csa-iot.org/wg/members-all/document/folder/3045[Causeway Link] | 19771ed      | 
...
| Matter 1.4.2     | v2.13-beta4.1+summer2025   | N/A                                                                                         |                                                                               | 871b434      | To install, use the tag "v2.13-beta4.1+summer2025" in the instructions of <<fresh_install>>
|===
...
WARNING: Known problem: The latest version of wpa_supplicant (v2.11 or later) may cause compatibility issues with certain Wi-Fi chipsets on Raspberry-Pi. This can result in the wireless connection intermittently dropping and reconnecting. 
Workaround to avoid the compatibility problem: 
Modify the boot configuration by adding the setting to /boot/firmware/cmdline.txt file:
console=serial0,115200 multipath=off dwc_otg.lpm_enable=0 console=tty1 root=LABEL=writable rootfstype=ext4 rootwait fixrtc brcmfmac.roamoff=1 brcmfmac.feature_disable=0x282000
...
** Stop the default Wi-Fi interface of raspberry-pi:
*** sudo ifconfig wlan0 down
*** sudo rmmod brcmfmac_wcc
*** sudo rmmod brcmfmac
** Stop the network manage:
...
[NOTE]
==== 
It's required to run on commissionable device before running each tests
==== 

=== Additional Trouble Shooting Helpful Notes
* *Commissionee failed to connect to AP in running PAF commissioning*:

**Symptom**: The messages are observed on the log:
==============================================
[1749462967.571] [3575:3576] [DL] wpa_supplicant:PropertiesChanged:key:State -> 'scanning'
[1749462969.775] [3575:3576] [DL] wpa_supplicant:PropertiesChanged:key:Scanning -> true
[1749462983.051] [3575:3576] [DL] wpa_supplicant: network scan done
[1749462983.544] [3575:3576] [DL] Network Found: (1a:27:f5:37:4e:c7) Signal:-49
[1749462983.549] [3575:3576] [DL] Network Found: Chip5 (dc:ea:e7:34:f5:5e) Signal:-51...

[1749462983.999] [3575:3576] [DL] Network Found: GRL-Public (c6:06:c3:f9:5e:ce) Signal:-61
[1749462984.004] [3575:3576] [DL] Network Found: Qualitrix (b0:19:21:73:ee:78) Signal:-77
[1749462984.008] [3575:3575] [PAF]Network resource has been unavailable for a long time
[1749462984.008] [3575:3575] [PAF]Shutdown PAF session (1, 0)
==============================================
**Reason**: There is a protect in SDK that it will stop sending PAF packets if device is scanning. The period is 15 seconds. If the environment is noisy, it's possible to hit the problem and failed to continue.

**Solution/WAR**: Need a cleaner environment. We will add patch to make this period configurable in SDK.

* *Device failed to run after upgrading to v2.10 wpa_supplicant*:

**Symptom**:Device can associate with AP, but failed at the last step if running Wi-Fi commissioning (including code-wifi, ble-wifi, wifipaf-wifi)

**Reason**:The problem will be observed if the `/etc/wpa_supplicant/wpa_supplicant.conf` has the ap information.

**Solution/WAR:ref**: Renew the commissionee environment before running the test in **WiFi PAF Commissioning Test Readme CSA Matter** file.
...
The Test Harness applies different rules to pre-select test cases depending on the type of certification being performed. The selection process considers specific flags present in the PICS file and, in certain cases—such as when PLAT.CERT or PLAT.CERT.TESTS.DONE PICS are present, it also evaluates the contents of additional configuration files, namely `generated-platform-cert-test-list.json` and `dmp-test-skip.xml`.
...
* The PICS file must contain the `PLAT.CERT` flag set to `True`.
* The TH will pre-select test cases that:
** Are explicitly listed in the `generated-platform-cert-test-list.json` file and also comply with the standard PICS rules.

==== Derived Product Certification
This mode applies to products built on a previously certified platform. The behavior is:

* The PICS file must contain the `PLAT.CERT.TESTS.DONE` flag set to `True`, along with any product-specific PICS entries.
* The TH will pre-select test cases that:
...
* The PICS file may contain any PICS entries, except `PLAT.CERT` and `PLAT.CERT.TESTS.DONE`.
...
* `PLAT.CERT = True` → Platform Certification
* `PLAT.CERT.TESTS.DONE = True` → Derived Product Certification
* Neither flag set → Full Product Certification

WARNING: PLAT.CERT and PLAT.CERT.TESTS.DONE are mutually exclusive. Please enable only one of them.
